...
#else
#ifdef ENABLE_LEDMAP
#ifdef USE_LOCAL_LEDMAP
static void IRAM_ATTR loadAndTranspose(uint8_t *ledt, int *sizes, int num_stripst, uint16_t *buffer, int ledtodisp, uint8_t *mapg, uint8_t *mapr, uint8_t *mapb, uint8_t *mapw, int nbcomponents, int pg, int pr, int pb, OffsetDisplay offdisp);
#else
static void IRAM_ATTR loadAndTranspose(uint8_t *ledt, int *sizes, int num_stripst, uint16_t *buffer, int ledtodisp, uint8_t *mapg, uint8_t *mapr, uint8_t *mapb, uint8_t *mapw, int nbcomponents, int pg, int pr, int pb, OffsetDisplay offdisp, Coordinates (*mapLed)(int));
#endif
// static void IRAM_ATTR loadAndTranspose(uint8_t *ledt, int *sizes, int num_stripst, uint16_t *buffer, int ledtodisp, uint8_t *mapg, uint8_t *mapr, uint8_t *mapb, uint8_t *mapw, int nbcomponents, int pg, int pr, int pb, Coordinates (*mapLed)(int), OffsetDisplay offdisp);
#else
...
static Coordinates defaultMapLed(int ledNum)
{
  Coordinates coordinates;
  coordinates.x = ledNum;
  coordinates.y = 0;
  return coordinates;
}
...
public:
    ...
    Coordinates (*mapLed)(int) = defaultMapLed;
    
    void setMapLed(Coordinates (*newMapLed)(int))
    {
      mapLed = newMapLed;
    }
...
//add the capability of going a bit faster
    ...
    void transposeAll()
    {
        ...
        for (int j = 0; j < num_led_per_strip; j++)
        {
            ...
            #ifdef ENABLE_LEDMAP
            #ifdef USE_LOCAL_LEDMAP
            loadAndTranspose(leds, stripSize, num_strips, (uint16_t *)DMABuffersTransposed[j+1]->buffer, ledToDisplay, __green_map, __red_map, __blue_map, __white_map, nb_components, p_g, p_r, p_b, _offsetDisplay);
            #else
            loadAndTranspose(leds, stripSize, num_strips, (uint16_t *)DMABuffersTransposed[j+1]->buffer, ledToDisplay, __green_map, __red_map, __blue_map, __white_map, nb_components, p_g, p_r, p_b, _offsetDisplay, mapLed);
            #endif
            #else
...
*/
    ...
    void showPixels()
    {
        showPixels(_defaultLeds, _offsetDisplay);
    }

    void showPixels(OffsetDisplay offdisp)
    {
        showPixels(_defaultLeds, offdisp);
    }
...
*/
    ...
    {
         showPixels(newleds, _offsetDisplay);
    }
    ...
    void showPixels(uint8_t *newleds, OffsetDisplay offdisp)
    {
        ...
        leds = newleds;
        #ifdef ENABLE_LEDMAP
        // _offsetDisplay.offsetx = MOD(_offsetDisplay.offsetx, PANEL_WIDTH * NUM_PANELS_PER_ROW);
        // _offsetDisplay.offsety = MOD(_offsetDisplay.offsety, PANEL_HEIGHT * NUM_PANELS_PER_COLUMN);
        // if (_offsetDisplay.offsetx < 0) _offsetDisplay.offsetx += (INT_MAX / (_offsetDisplay.panel_width)) * (_offsetDisplay.panel_width);
        // if (_offsetDisplay.offsety < 0) _offsetDisplay.offsety += (INT_MAX / (_offsetDisplay.panel_height)) * (_offsetDisplay.panel_height);
        if (_offsetDisplay.offsetx < 0) _offsetDisplay.offsetx -= (_offsetDisplay.offsetx / _offsetDisplay.panel_width - 1) * _offsetDisplay.panel_width;
        if (_offsetDisplay.offsety < 0) _offsetDisplay.offsety -= (_offsetDisplay.offsety / _offsetDisplay.panel_height - 1) * _offsetDisplay.panel_height;
        #endif
        #if HARDWARESPRITES == 1
        ...
        #ifdef ENABLE_LEDMAP
        #ifdef USE_LOCAL_LEDMAP
        loadAndTranspose(leds, stripSize, num_strips, (uint16_t *)DMABuffersTampon[0]->buffer, ledToDisplay, __green_map, __red_map, __blue_map, __white_map, nb_components, p_g, p_r, p_b, _offsetDisplay);
        #else
        loadAndTranspose(leds, stripSize, num_strips, (uint16_t *)DMABuffersTampon[0]->buffer, ledToDisplay, __green_map, __red_map, __blue_map, __white_map, nb_components, p_g, p_r, p_b, _offsetDisplay, mapLed);
        #endif
        #else
        ...
        isWaiting = true;
        if (__displayMode != NO_WAIT) waitUntilDone();
    }
    ...
    if (GET_PERI_REG_BITS(I2S_INT_ST_REG(I2S_DEVICE), I2S_OUT_EOF_INT_ST_V, I2S_OUT_EOF_INT_ST_S))
    {
        ...
        if (((I2SClocklessLedDriver *)arg)->transpose)
        {
            ...
            if (cont->ledToDisplay < cont->num_led_per_strip)
            {
                ...
                #ifdef ENABLE_LEDMAP
                #ifdef USE_LOCAL_LEDMAP
                loadAndTranspose(cont->leds, cont->stripSize, cont->num_strips, (uint16_t *)cont->DMABuffersTampon[cont->dmaBufferActive]->buffer, cont->ledToDisplay, cont->__green_map, cont->__red_map, cont->__blue_map, cont->__white_map, cont->nb_components, cont->p_g, cont->p_r, cont->p_b, cont->_offsetDisplay);
                #else
                loadAndTranspose(cont->leds, cont->stripSize, cont->num_strips, (uint16_t *)cont->DMABuffersTampon[cont->dmaBufferActive]->buffer, cont->ledToDisplay, cont->__green_map, cont->__red_map, cont->__blue_map, cont->__white_map, cont->nb_components, cont->p_g, cont->p_r, cont->p_b, cont->_offsetDisplay, cont->mapLed);
                #endif
                #else
...
#ifdef ENABLE_LEDMAP
#ifdef USE_LOCAL_LEDMAP
__attribute__((always_inline)) IRAM_ATTR static Coordinates mapLedLocal(int hardwareLed)
{
  Coordinates coordinates;
  // = MOD(FLOOR(A11 / 4), 12)
...
{
    Lines secondPixel[nbcomponents];
    int currentLed = ledtodisp;
    for (int i = 0; i < num_stripst; i++)
    {
        #ifdef USE_LOCAL_LEDMAP
        Coordinates coordinates = mapLedLocal(currentLed);
        #else
        Coordinates coordinates = mapLed(currentLed);
        #endif
        coordinates.x = (coordinates.x + offdisp.offsetx) % offdisp.panel_width;
        coordinates.y = (coordinates.y + offdisp.offsety) % offdisp.panel_height;
        uint8_t *poli = ledt + coordinates.x * nbcomponents + coordinates.y * offdisp.panel_width * nbcomponents;
